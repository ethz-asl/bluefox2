...
private:
    ...
    wxString                                    GetProductFromManufacturerInfo( mvIMPACT::acquire::Device* pDev ) const;
    bool                                        isBlueCOUGAR_X_9XAS1( mvIMPACT::acquire::Device* pDev ) const
    {
        const wxString token = GetProductFromManufacturerInfo( pDev );
        return token.StartsWith( wxT( "X" ) ) && token.Contains( wxT( "9XAS1" ) );
    }
    bool                                        isBlueFOX3_9XAS1( mvIMPACT::acquire::Device* pDev ) const
    {
        const wxString token = GetProductFromManufacturerInfo( pDev );
