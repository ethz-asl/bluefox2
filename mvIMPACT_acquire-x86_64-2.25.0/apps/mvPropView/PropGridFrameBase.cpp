...
//-----------------------------------------------------------------------------
{
    ...
    if( pProp )
    {
        ...
        if( pMethod )
        {
            ...
            {
                wxString errorString( wxString::Format( wxT( "An error occurred while executing function '%s':\n\nError string: %s\n\nError code: %d(%s))." ), pMethod->FriendlyName().c_str(), ConvertedString( ExceptionFactory::getLastErrorString() ).c_str(), callResult, ImpactAcquireException::getErrorCodeAsString( callResult ).c_str() ) );
