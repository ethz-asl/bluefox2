...
//-----------------------------------------------------------------------------
{
    ...
    string manufacturer( pDev->manufacturer.isValid() ? pDev->manufacturer.read() : "" );
    transform( manufacturer.begin(), manufacturer.end(), manufacturer.begin(), ptr_fun<int, int>( tolower ) );
    if( ( ( ( product.find( "mvBlueFOX3" ) != string::npos ) || ( product.find( "mvBlueCOUGAR-X" ) != string::npos ) ) && ( manufacturer.find( "matrix vision" ) != string::npos ) ) ||
        isBlueCOUGAR_X_9XAS1( pDev ) ||
        isBlueFOX3_9XAS1( pDev ) )
    {
...
//-----------------------------------------------------------------------------
wxString DevicePropertyHandler::GetProductFromManufacturerInfo( mvIMPACT::acquire::Device* pDev ) const
//-----------------------------------------------------------------------------
{
    wxString product;
