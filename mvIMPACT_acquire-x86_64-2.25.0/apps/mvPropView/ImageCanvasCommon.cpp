...
//-----------------------------------------------------------------------------
void ImageCanvas::BlitAOI( wxPaintDC& dc, double scaleFactor, int bmpXOff, int bmpYOff, int bmpW, int bmpH, const AOI* pAOI, bool boDrawOnTop ) const
//-----------------------------------------------------------------------------
{
    if( pAOI->m_boDrawOnTop == boDrawOnTop )
    {
        ...
        for( wxArrayString::size_type i = 0; i < labelTokenCount; i++ )
        {
            ...
            dc.DrawText( labelTokens[i],
                         ...
                         static_cast<wxCoord>( ( ( aoiy * scaleFactor ) + bmpYOff + ( ( static_cast<int>( aoih * scaleFactor ) - textHeight ) / 2 ) ) ) - ( ( labelTokenCount * textHeight ) / 2 ) + ( i * textHeight ) );
        }
        dc.SetPen( wxNullPen );
    }
}
...
//-----------------------------------------------------------------------------
void ImageCanvas::BlitSizingPoints( wxPaintDC& dc, double scaleFactor, int bmpXOff, int bmpYOff ) const
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
void ImageCanvas::BlitAOIs( wxPaintDC& dc, double scaleFactor, int bmpXOff, int bmpYOff, int bmpW, int bmpH ) const
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
void ImageCanvas::BlitAOIs( wxPaintDC& dc, double scaleFactor, int bmpXOff, int bmpYOff, int bmpW, int bmpH, bool boDrawOnTop ) const
//-----------------------------------------------------------------------------
{
            ...
            if( e.LeftIsDown() )
            {
                ...
                else
                {
                    ...
                    if( !IsScaled() )
                    {
                     ...
                     m_pActiveAnalysisPlot && m_pActiveAnalysisPlot->IsActive() && !m_pActiveAnalysisPlot->IsAOIFullModeActive() &&
                     ( m_aoiContainer.size() == 1 ) ) // as the right mouse key is used to completely redraw an existing AOI this can only work with a single AOI. Otherwise how to choose which one to delete?
            {
