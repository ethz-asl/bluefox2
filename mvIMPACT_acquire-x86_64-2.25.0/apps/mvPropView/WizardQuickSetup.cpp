...
WizardQuickSetup::WizardQuickSetup( PropViewFrame* pParent, const wxString& title, Device* pDev, FunctionInterface* pFI, bool boAutoConfigureOnStart )
    : OkAndCancelDlg( pParent, widMainFrame, title, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxMINIMIZE_BOX ),
      ...
      pFI_( pFI ), pIP_( 0 ), boLastWBChannelRead_( wbcRed ), pParentPropViewFrame_( pParent ), propertyChangeHistory_(), pID_( 0 )
//-----------------------------------------------------------------------------
{
    /*
        |-------------------------------------|
    ...
    // 'Presets' controls
    pBtnPresetColor_ = new wxBitmapButton( pPanel, widBtnPresetColor, *GlobalDataStorage::Instance()->GetBitmap( GlobalDataStorage::bIcon_ColorPreset ), wxDefaultPosition, wxDefaultSize, wxBU_AUTODRAW, wxDefaultValidator, wxT( "High-Quality color" ) );
    pBtnPresetColor_->SetToolTip( wxT( "Will setup the device for color image capture.\nCurrent settings will be overwritten!" ) );
    pBtnPresetColor_->SetBitmapDisabled( *GlobalDataStorage::Instance()->GetBitmap( GlobalDataStorage::bIcon_ColorPreset_Disabled ) );
...
//-----------------------------------------------------------------------------
{
    ...
    pID_->restoreDefault();
    pIP_->restoreDefault();
    QueryInitialDeviceSettings( currentSettings_ );
    if( featuresSupported_.boColorOptionsSupport )
    {
        currentSettings_.boColorEnabled = true;
    }
...
//-----------------------------------------------------------------------------
{
    if( boGUILocked_ == false )
    {
        try
        {
            if( featuresSupported_.boPlatformIPPSuport )
            {
...
//-----------------------------------------------------------------------------
{
    ...
    devSettings.optimizationLevel = olBestQuality;

    currentSettings_ = devSettings;
    SupportedWizardFeatures supportedFeatures;
    AnalyzeDeviceAndGatherInformation( supportedFeatures );
    featuresSupported_ = supportedFeatures;
    SetupDriverSettings();
...
//-----------------------------------------------------------------------------
{
    SupportedWizardFeatures features = featuresSupported_;
    DeviceSettings settings = currentSettings_;
...
{
    SupportedWizardFeatures features = featuresSupported_;
    DeviceSettings devSettings = currentSettings_;
...
//-----------------------------------------------------------------------------
{
    ...
    if( featuresSupported_.boColorOptionsSupport && featuresSupported_.boPlatformIPPSuport )
    {
...
//-----------------------------------------------------------------------------
void WizardQuickSetup::SetupOptimisation( void )
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
{
    ...
    if( ( featuresSupported_.boAutoWhiteBalanceSupport &&
          currentSettings_.boAutoWhiteBalanceEnabled &&
          currentSettings_.boColorEnabled ) ||
...
//-----------------------------------------------------------------------------
{
    ...
    if( find( itBEGIN, itEND, rg ) != itEND )
    {
        ...
        currentSettings_.imageFormatControlPixelFormat = rg;
    }
    else if( find( itBEGIN, itEND, gr ) != itEND )
    {
        LoggedWriteProperty( pIFC_->pixelFormat, gr );
        currentSettings_.imageFormatControlPixelFormat = gr;
    }
    else if( find( itBEGIN, itEND, bg ) != itEND )
    {
        LoggedWriteProperty( pIFC_->pixelFormat, bg );
        currentSettings_.imageFormatControlPixelFormat = bg;
    }
    else if( find( itBEGIN, itEND, gb ) != itEND )
    {
        LoggedWriteProperty( pIFC_->pixelFormat, gb );
        currentSettings_.imageFormatControlPixelFormat = gb;
    }
    else
    {
        if( find( itBEGIN, itEND, defaultValue ) != itEND )
        {
            LoggedWriteProperty( pIFC_->pixelFormat, defaultValue );
            currentSettings_.imageFormatControlPixelFormat = defaultValue;
        }
        else
        {
            const string oneExistingValue = pixelFormatStrings.front();
            LoggedWriteProperty( pIFC_->pixelFormat, oneExistingValue );
            currentSettings_.imageFormatControlPixelFormat = oneExistingValue;
        }
    }
}
...
//-----------------------------------------------------------------------------
{
    ...
    if( find( pixelFormatStrings.begin(), itEND, mono ) != itEND )
    {
        ...
        currentSettings_.imageFormatControlPixelFormat = mono;
    }
    else
    {
...
//-----------------------------------------------------------------------------
{
    const string sensorName( pDeC_->mvDeviceSensorName.readS() );
    if( pDeC_->mvDeviceClockFrequency.isValid() )
    {
        ...
        if( pixelClocks.size() > 1 )
        {
            ...
            const int64_type optimalPixelClock = boHighQuality ? ( sensorName.find( "MT9" ) == 0 ? pixelClocks[1] : pixelClocks[0] ) : pixelClocks[pixelClocks.size() - 1];
...
//-----------------------------------------------------------------------------
{
    if( boEnable )
    {
        LoggedWriteProperty( pAcC_->exposureAuto, std::string( "Continuous" ) );
    ...
    {
        LoggedWriteProperty( pAcC_->exposureAuto, std::string( "Off" ) );
    }
}
...
//-----------------------------------------------------------------------------
{
    if( boEnable )
    {
        ...
        {
            // We do this to eliminate any residual digital gain values, since we switch to autogain which is analog, and we
            // do not need any digital offset to carry with us.
            DoWriteUnifiedGain( 0. );
        }
        if( pAnC_->gainSelector.readS() != "AnalogAll" )
        {
            pAnC_->gainSelector.writeS( "AnalogAll" );
        }
    ...
    else
    {
        ...
        LoggedWriteProperty( pAnC_->gainAuto, string( "Off" ) );
    }
...
//-----------------------------------------------------------------------------
{
    if( pAcC_->acquisitionFrameRate.isValid() && pAcC_->mvAcquisitionFrameRateLimitMode.isValid() )
    {
        ...
        if( pAcC_->mvAcquisitionFrameRateLimitMode.readS() != string( "mvDeviceLinkThroughput" ) )
        {
            LoggedWriteProperty( pAcC_->mvAcquisitionFrameRateLimitMode, string( "mvDeviceLinkThroughput" ) );
        }
...
//-----------------------------------------------------------------------------
{
        ...
        boHighQuality )
    {
        ...
        currentSettings_.imageFormatControlPixelFormat = m10;
    }
    else if( find( itBEGIN, itEND, m8 ) != itEND )
    {
        LoggedWriteProperty( pCSBF_->pixelFormat, m8 );
        currentSettings_.imageFormatControlPixelFormat = m8;
    }
    else
    {
...
//-----------------------------------------------------------------------------
{
    ...
    {
        LoggedWriteProperty( pCSBF_->autoExposeControl, std::string( "Off" ) );
    }
}
