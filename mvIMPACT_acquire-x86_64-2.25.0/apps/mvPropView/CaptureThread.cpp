...
CaptureThread::CaptureThread( FunctionInterface* pFuncInterface, Device* pDev, wxWindow* pParentWindow, unsigned int pendingImageQueueDepth ) :
    ...
    m_percentageOfImagesSentToDisplay( 100. ), m_captureQueueDepth( 0 ), m_currentlyActiveRequests( 0 ), m_bufferPartIndex( 0 ),
    m_multipleImageBuffer( 1 ), m_multipleImageBufferImageCount( 1 ), m_multipleImageBufferLineOffset( 0 )
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
void CaptureThread::GetImageData( RequestData& requestData, const TImageDataRetrievalMode mode ) const
//-----------------------------------------------------------------------------
{
    wxCriticalSectionLocker locker( m_critSect );
    ...
    if( ( mode == idrmRecordedSequence ) && ( m_currentSequenceIndexToDisplay < m_recordedSequence.size() ) && m_recordedSequence[m_currentSequenceIndexToDisplay].second )
    {
        requestData = *m_recordedSequence[m_currentSequenceIndexToDisplay].second;
        if( m_multipleImageBufferImageCount > 1 )
        {
            UpdateMultiImageBuffer( m_recordedSequence[m_currentSequenceIndexToDisplay].second->image_ );
            requestData.image_ = m_multipleImageBuffer;
        }
    }
    else
    {
        GetRequestData( requestData, m_currentRequest, false );
    }
}
...
//-----------------------------------------------------------------------------
void CaptureThread::GetRequestData( RequestData& requestData, const int requestNr, const bool boDeepCopy ) const
//-----------------------------------------------------------------------------
{
    ...
    if( requestNr != INVALID_ID )
    {
        ...
        if( pReq )
        {
            if( m_multipleImageBufferImageCount > 1 )
            {
                UpdateMultiImageBuffer( pReq->getImageBufferDesc() );
            }
            ...
            const unsigned int bufferPartCount = pReq->getBufferPartCount();
            if( bufferPartCount == 0 )
            {
                const ImageBufferDesc& nbd( ( m_multipleImageBufferImageCount > 1 ) ? m_multipleImageBuffer : pReq->getImageBufferDesc() );
                requestData.image_ = boDeepCopy ? nbd.clone() : nbd;
                GetRequestData_BayerInfo( requestData, pReq->imageBayerMosaicParity, pReq->imagePixelFormat );
            }
            else
            {
                requestData.bufferPartIndex_ = ( m_bufferPartIndex < bufferPartCount ) ? m_bufferPartIndex : 0;
                BufferPart& part = pReq->getBufferPart( requestData.bufferPartIndex_ );
                const TBufferPartDataType dataType = part.dataType.read();
                switch( dataType )
                {
                case bpdt2DImage:
                ...
                case bpdtConfidenceMap:
                    {
                        const ImageBufferDesc& nbd( ( m_multipleImageBufferImageCount > 1 ) ? m_multipleImageBuffer : part.getImageBufferDesc() );
                        requestData.image_ = boDeepCopy ? nbd.clone() : nbd;
                        GetRequestData_BayerInfo( requestData, part.bayerMosaicParity, part.pixelFormat );
                    }
                    break;
                case bpdtJPEG:
                case bpdtJPEG2000:
                    {
                        // corrupt JPEG images will result in an annoying message box if logging is switched on...
                        wxLogNull logSuspendScope;
                        wxMemoryInputStream JPEGStream( part.address.read(), part.dataSize.read() );
                        wxImage image;
                        if( image.LoadFile( JPEGStream, wxBITMAP_TYPE_JPEG ) )
                        {
                            SetRequestDataFromWxImage( requestData, image );
                        }
                        else
                        {
                            requestData.requestInfo_.requestResult_ = rrFrameCorrupt;
                        }
                    }
                    break;
                case bpdtUnknown:
...
void CaptureThread::GetRequestData_BayerInfo( RequestData& requestData, _Ty& imageBayerMosaicParity, _Tx& imagePixelFormat ) const
//-----------------------------------------------------------------------------
{
    requestData.bayerParity_ = static_cast<TBayerMosaicParity>( imageBayerMosaicParity.read() );
    if( requestData.bayerParity_ != bmpUndefined )
    {
        requestData.pixelFormat_ = wxString::Format( wxT( "%s(BayerPattern=%s)" ), ConvertedString( imagePixelFormat.readS() ).c_str(), ConvertedString( imageBayerMosaicParity.readS() ).c_str() );
    }
    else
    {
        requestData.pixelFormat_ = ConvertedString( imagePixelFormat.readS() );
    }
}
...
//-----------------------------------------------------------------------------
void CaptureThread::SetImagesPerDisplayCount( long imagesPerDisplayCount )
//-----------------------------------------------------------------------------
{
    wxCriticalSectionLocker locker( m_critSect );
    m_multipleImageBufferImageCount = imagesPerDisplayCount;
    m_multipleImageBuffer = ImageBufferDesc( 1 );
}
...
//-----------------------------------------------------------------------------
void CaptureThread::UpdateMultiImageBuffer( const ImageBufferDesc& desc ) const
//-----------------------------------------------------------------------------
{
    if( m_multipleImageBufferImageCount > 1 )
    {
        const TImageBufferPixelFormat pixelFormat = desc.getBuffer()->pixelFormat;
        const int height = desc.getBuffer()->iHeight;
        const int channelCount = desc.getBuffer()->iChannelCount;
        if( ( m_multipleImageBuffer.getBuffer()->pixelFormat != pixelFormat ) ||
            ( m_multipleImageBuffer.getBuffer()->iHeight < height ) ||
            ( m_multipleImageBuffer.getBuffer()->pChannels[0].iLinePitch != desc.getBuffer()->pChannels[0].iLinePitch ) )
        {
            m_multipleImageBuffer = ImageBufferDesc( pixelFormat, desc.getBuffer()->iWidth, height * m_multipleImageBufferImageCount );
            memset( m_multipleImageBuffer.getBuffer()->vpData, 0, m_multipleImageBuffer.getBuffer()->iSize );
            m_multipleImageBufferLineOffset = 0;
        }
        if( ( height + m_multipleImageBufferLineOffset ) > m_multipleImageBuffer.getBuffer()->iHeight )
        {
            m_multipleImageBufferLineOffset = 0;
        }

        const char* pData = static_cast<const char*>( desc.getBuffer()->vpData );
        for( int channel = 0; channel < channelCount; channel++ )
        {
            memcpy( static_cast<char*>( m_multipleImageBuffer.getBuffer()->vpData ) + m_multipleImageBuffer.getBuffer()->pChannels[channel].iChannelOffset + m_multipleImageBufferLineOffset * m_multipleImageBuffer.getBuffer()->pChannels[channel].iLinePitch,
                    pData + desc.getBuffer()->pChannels[channel].iChannelOffset,
                    height * desc.getBuffer()->pChannels[channel].iLinePitch );
        }
        m_multipleImageBufferLineOffset += height;
    }
}
