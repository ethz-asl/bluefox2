...
//-----------------------------------------------------------------------------
{
    if( m_GridItemId && GetComponent().isMeth() )
    {
        ...
        try
        {
            ...
            const string delimiterANSI( m_ParamDelimiter.mb_str() );
            callResult = meth.call( paramsANSI, delimiterANSI );
        ...
        catch( const ImpactAcquireException& e )
        {
            ...
            wxString errorString( wxString::Format( wxT( "An error occurred while executing function '%s'(actual driver feature name: '%s'):\n\n%s(numerical error representation: %d (%s))." ), m_FriendlyName.c_str(), ConvertedString( GetComponent().name() ).c_str(), ConvertedString( e.getErrorString() ).c_str(), e.getErrorCode(), ConvertedString( e.getErrorCodeAsString() ).c_str() ) );
...
//-----------------------------------------------------------------------------
{
    ...
    {
        PropertyI( comp ).write( ntohl( inet_addr( value.c_str() ) ), index );
    }
    else if( ( representation == crIPv4Address ) && ( type == ctPropInt64 ) )
    {
        PropertyI64( comp ).write( static_cast<int64_type>( ntohl( inet_addr( value.c_str() ) ) ), index );
    }
    else if( ( representation == crMACAddress ) && ( type == ctPropInt64 ) )
    {
