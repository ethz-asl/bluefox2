...
//-----------------------------------------------------------------------------
{
    ...
    case pgBlueCOUGAR_S:
        ...
        break;
    case pgBlueCOUGAR_X:
        if( isBlueCOUGAR_X_9XAS1() )
        {
            firmwareUpdateFileName_ = wxString( wxT( "BVS_CA_Update.mvu" ) );
            productStringForFirmwareUpdateCheck_ = wxString( wxT( "BVS_CA" ) );
        }
        else
        {
...
//-----------------------------------------------------------------------------
{
    ...
    if( it == itEND )
    {
        it = entry.suitableProductKeys_.find( product );
    }
    return it;
}
...
//-----------------------------------------------------------------------------
{
    ...
    if( updateBootProgrammer.isValid() )
    {
        ...
        static const int MAX_UPDATE_TIME_MILLISECONDS = 40000;
        static const int UPDATE_THREAD_PROGRESS_INTERVAL_MILLISECONDS = 100;
        wxProgressDialog updateBootProgrammerDlg( wxT( "Updating Boot Programmer In Flash" ),
                wxT( "Please wait...                                " ),
                MAX_UPDATE_TIME_MILLISECONDS, // range
                pParent_,     // parent
                wxPD_AUTO_HIDE | wxPD_APP_MODAL | wxPD_ELAPSED_TIME );
        ...
        {
            wxMilliSleep( UPDATE_THREAD_PROGRESS_INTERVAL_MILLISECONDS );
            updateBootProgrammerDlg.Pulse();
        }
        updateThread.Wait();
        updateBootProgrammerDlg.Update( MAX_UPDATE_TIME_MILLISECONDS );
    ...
    else
    {
        ...
        if( ( choices.size() > 1 ) &&
            !boSilentMode )
        {
            wxSingleChoiceDialog dlg( pParent_, wxT( "This archive contains more than one firmware file matching the product name.\nPlease select a file you want to program the device with." ), wxT( "Please select a file to upload and install on the device" ), choices );
            if( dlg.ShowModal() == wxID_OK )
            {
                selection = dlg.GetStringSelection().BeforeFirst( wxT( '(' ) );
            }
...
//-----------------------------------------------------------------------------
void DeviceHandlerBlueDevice::WaitForDeviceToReappear( const wxString& serial )
//-----------------------------------------------------------------------------
{
