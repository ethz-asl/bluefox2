...
//-----------------------------------------------------------------------------
{
    ...
    bool                                isBlueCOUGAR_X_9XAS1( void ) const
    {
        const wxString token = GetProductFromManufacturerInfo();
        return token.StartsWith( wxT( "X" ) ) && token.Contains( wxT( "9XAS1" ) );
    }
    bool                                IsBlueCOUGAR_Y( void ) const;
    bool                                IsBlueFOX3( void ) const
    {
        return( ( ConvertedString( pDev_->family.read() ).Find( wxT( "mvBlueFOX3" ) ) == wxNOT_FOUND ) &&
                !isBlueFOX3_9XAS1() ) ? false : true;
    }
    bool                                isBlueFOX3_9XAS1( void ) const
    {
        const wxString token = GetProductFromManufacturerInfo();
        return token.StartsWith( wxT( "2" ) ) && token.Contains( wxT( "9XAS1" ) );
    }
