...
DeviceConfigureFrame::DeviceConfigureFrame( const wxString& title, const wxPoint& pos, const wxSize& size, int argc, wxChar** argv )
    : wxFrame( ( wxFrame* )NULL, wxID_ANY, title, pos, size ), m_pSystemModule( 0 ), m_boDevicesOnIgnoredInterfecesDetected( false ),
      m_commandLineIgnoreGEV( iiclpNotConfigured ), m_commandLineIgnoreU3V( iiclpNotConfigured ), m_ignoredInterfacesInfo(), m_pDevListCtrl( 0 ),
      m_pLogWindow( 0 ), m_logFileName(), m_lastDevMgrChangedCount( numeric_limits<unsigned int>::max() ), m_customFirmwareFile(),
      m_customFirmwarePath(), m_customGenICamFile(), m_IPv4Masks(), m_boPendingQuit( false ), m_boForceFirmwareUpdate( false )
#ifdef BUILD_WITH_PROCESSOR_POWER_STATE_CONFIGURATION_SUPPORT
...
//-----------------------------------------------------------------------------
{
    ...
    m_deviceHandlerFactory.Register( wxString( wxT( "mvBlueSIRIUS" ) ), DeviceHandlerBlueDevice::Create );
    m_deviceHandlerFactory.Register( wxString( wxT( "mvHYPERION" ) ), DeviceHandlerHYPERION::Create );
    m_deviceHandlerFactory.Register( wxString( wxT( "BVS CA" ) ), DeviceHandlerBlueDevice::Create );
...
#   endif // #if wxCHECK_VERSION(2, 7, 1)
    ...
    WriteLogMessage( wxT( "'ignore_gev'                or 'igev' to disable enumerating devices on all GEV interfaces of the system, for all applications using the mvGenTLProducer (syntax: 'igev=1' or 'igev=0').\n" ) );
    WriteLogMessage( wxT( "'ignore_u3v'                or 'iu3v' to disable enumerating devices on all U3V interfaces of the system, for all applications using the mvGenTLProducer (syntax: 'iu3v=1' or 'iu3v=0').\n" ) );
    ...
    WriteLogMessage( wxT( "mvDeviceConfigure igev=1 (will disable enumerating devices on all GEV interfaces of the system)\n" ) );
    WriteLogMessage( wxT( "\n" ) );
    if( argc > 1 )
    {
...
//-----------------------------------------------------------------------------
{
            ...
            try
            {
                ...
                if( interfaceID.isValid() && deviceMACAddress.isValid() )
                {
                    ...
                    for( int64_type interfaceIndex = 0; interfaceIndex < interfaceCount; interfaceIndex++ )
                    {
                        ...
                        m_pSystemModule->interfaceSelector.write( interfaceIndex );
                        const string interfaceDeviceUpdateListBehaviour = m_ignoredInterfacesInfo[interfaceDeviceHasBeenFoundOn];
                        if( ( boIgnoreGEVInterfaces && ( im.interfaceType.readS() == "GEV" ) && ( interfaceDeviceUpdateListBehaviour != "ForceEnumerate" ) ) ||
                            ( boIgnoreU3VInterfaces && ( im.interfaceType.readS() == "U3V" ) && ( interfaceDeviceUpdateListBehaviour != "ForceEnumerate" ) ) ||
    ...
    m_boDevicesOnIgnoredInterfecesDetected = devicesOnIgnoredInterfacesCnt != 0;
...
//-----------------------------------------------------------------------------
{
    ...
    if( m_boDevicesOnIgnoredInterfecesDetected )
    {
...
//-----------------------------------------------------------------------------
{
    ...
    if( m_pSystemModule && m_pSystemModule->mvDeviceUpdateListBehaviour.isValid() )
    {
        UnIgnoreInterfaceTechnology( "GEV" );
        UnIgnoreInterfaceTechnology( "U3V" );
        const unsigned int interfaceCount = m_pSystemModule->interfaceSelector.getMaxValue() + 1;
        for( unsigned int i = 0; i < interfaceCount; i++ )
        {
            m_pSystemModule->interfaceSelector.write( i );
            m_ignoredInterfacesInfo.insert( make_pair( m_pSystemModule->interfaceID.readS(), m_pSystemModule->mvDeviceUpdateListBehaviour.readS() ) );
            m_pSystemModule->mvDeviceUpdateListBehaviour.writeS( "NotConfigured" );
        }
    }
}
...
//-----------------------------------------------------------------------------
{
    if( m_pSystemModule && m_pSystemModule->mvInterfaceTechnologyToIgnoreSelector.isValid() &&
        m_pSystemModule->mvInterfaceTechnologyToIgnoreEnable.isValid() && m_pSystemModule->mvDeviceUpdateListBehaviour.isValid() )
    {
        m_pSystemModule->mvInterfaceTechnologyToIgnoreSelector.writeS( "GEV" );
        if( m_commandLineIgnoreGEV == iiclpNotConfigured )
        {
            m_pSystemModule->mvInterfaceTechnologyToIgnoreEnable.write( ( m_ignoredInterfacesInfo[string( "GEV" )] == string( "1" ) ) ? bTrue : bFalse );
        }
        else
        {
            m_pSystemModule->mvInterfaceTechnologyToIgnoreEnable.write( m_commandLineIgnoreGEV == iiclpEnableIgnore ? bTrue : bFalse );
        }
        m_pSystemModule->mvInterfaceTechnologyToIgnoreSelector.writeS( "U3V" );
        if( m_commandLineIgnoreU3V == iiclpNotConfigured )
        {
            m_pSystemModule->mvInterfaceTechnologyToIgnoreEnable.write( ( m_ignoredInterfacesInfo[string( "U3V" )] == string( "1" ) ) ? bTrue : bFalse );
        }
        else
        {
            m_pSystemModule->mvInterfaceTechnologyToIgnoreEnable.write( m_commandLineIgnoreU3V == iiclpEnableIgnore ? bTrue : bFalse );
        }
        ...
        {
            m_pSystemModule->interfaceSelector.write( i );
            m_pSystemModule->mvDeviceUpdateListBehaviour.writeS( m_ignoredInterfacesInfo[m_pSystemModule->interfaceID.readS()] );
        }
    }
}
...
//-----------------------------------------------------------------------------
void DeviceConfigureFrame::UnIgnoreInterfaceTechnology( const string& technology )
//-----------------------------------------------------------------------------
{
    if( m_pSystemModule->mvInterfaceTechnologyToIgnoreSelector.isValid() && m_pSystemModule->mvInterfaceTechnologyToIgnoreEnable.isValid() )
    {
        m_pSystemModule->mvInterfaceTechnologyToIgnoreSelector.writeS( technology );
        if( m_pSystemModule->mvInterfaceTechnologyToIgnoreEnable.read() == bTrue )
        {
            m_ignoredInterfacesInfo.insert( make_pair( technology, m_pSystemModule->mvInterfaceTechnologyToIgnoreEnable.readS() ) );
        }
        m_pSystemModule->mvInterfaceTechnologyToIgnoreEnable.write( bFalse );
    }
}
